<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="布置时间" prop="buzhishijian">
          <el-input v-model="ruleForm.buzhishijian" 
              placeholder="布置时间" clearable  :readonly="ro.buzhishijian"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="布置时间" prop="buzhishijian">
              <el-input v-model="ruleForm.buzhishijian" 
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="需完成时间" prop="xuwanchengshijian">
          <el-input v-model="ruleForm.xuwanchengshijian" 
              placeholder="需完成时间" clearable  :readonly="ro.xuwanchengshijian"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="需完成时间" prop="xuwanchengshijian">
              <el-input v-model="ruleForm.xuwanchengshijian" 
...
export default {
  ...
  methods: {
    ...
    info(id) {
      this.$http({
        url: `zuoyewanchengxinxi/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
	...
	this.ruleForm.wanchengneirong = this.ruleForm.wanchengneirong.replace(reg,'../../../ssm7i245/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `zuoyewanchengxinxi/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.zuoyewanchengxinxiCrossAddOrUpdateFlag = false;
                  this.parent.search();
